...
/**
 * UI Frame for the Armor Table that the user uses 
 * to interact with the DBMS
 * @author adam Standke
 *
 */
public class Frame_ArmorUI extends Application implements Frame, ActionListener 
{
	...
	private JTextField aridTextField;
	private JTextField gidTextField;
	private JTextField protectionTextField;
	private JTextField armorTextField;
	private JTextField imageTextField; 
	private JTextField descriptionTextField; 
	private JTextField aridUpdateTextField;
	private JTextField gidUpdateTextField;
	private JTextField protectionUpdateTextField;
	private JTextField armorUpdateTextField;
	...
	/**
	 * Method that builds the actual 
	 * user interface for the Armor table
	 * that interacts with the DBMS
	 */
	@Override
	public void buildFrame() 
	{
		...
		frame.setTitle("Armor Interface");//sets title of frame
		...
		insertPanel.setLayout(new GridLayout(5,4));
				...
				String dataTypes = "DATA Types:\nArID(PK) INT(4); GID(FK) INT(4);\n"+
				"Protection_Amount INT(3); Armor_Location VARCHAR(50);\n Image BLOB(20M);"+
				"Description TEXT(1k)";
				...
				String insert = "INSERT Insrutctions:\nTo insert into ARMOR all boxes have to be "
						+ "filled.\n Fill boxes with the "
				...
				String update = "UPDATE Instructions:\nYou can update either one or all attributes in ARMOR by "
				...
				String delete ="DELETE Instructions:\nTo delete a row in ARMOR just use the WHERE text field "
				...
				String query = "QUERY Instructions:\nTo do a SQL Query on the ARMOR TABLE the form must be of "
						+ "the following:\n SELECT<attribute list> FROM ARMOR WHERE <condition>.\nTo simply display "
						+ "the rows of the ARMOR TABLE type the following"
				...
				JLabel lblarid = new JLabel("ARID:");
				lblarid.setFont(new Font("Tahoma", Font.PLAIN, 15));
				insertPanel.add(lblarid);
				aridTextField = new JTextField();
				insertPanel.add(aridTextField);
				aridTextField.setColumns(10);
				...
				JLabel lblamount = new JLabel("Protection Amount:");
				lblamount.setFont(new Font("Tahoma", Font.PLAIN, 15));
				insertPanel.add(lblamount);
				protectionTextField = new JTextField();
				protectionTextField.setColumns(10);
				insertPanel.add(protectionTextField);
				JLabel lbllocation = new JLabel("Armor Location:");
				...
				insertPanel.add(armorTextField);
				...
				//dummy spacing values to get consistent look
				insertPanel.add(new JLabel(""));
				insertPanel.add(new JLabel(""));
				insertPanel.add(new JLabel(""));
		...
		updatePanel.setLayout(new GridLayout(5,4));
				...
				JLabel lblupdateArid = new JLabel("ARID:");
				lblupdateArid.setFont(new Font("Tahoma", Font.PLAIN, 15));
				updatePanel.add(lblupdateArid);
				aridUpdateTextField = new JTextField();
				aridUpdateTextField.setColumns(10);
				updatePanel.add(aridUpdateTextField);
				...
				JLabel lblupdateprotection = new JLabel("Protection Amount:");
				lblupdateprotection.setFont(new Font("Tahoma", Font.PLAIN, 15));
				updatePanel.add(lblupdateprotection);
				protectionUpdateTextField = new JTextField();
				protectionUpdateTextField.setColumns(10);
				updatePanel.add(protectionUpdateTextField);
				JLabel lblupdatelocation = new JLabel("Armor Location:");
				lblupdatelocation.setFont(new Font("Tahoma", Font.PLAIN, 15));
				updatePanel.add(lblupdatelocation);
				armorUpdateTextField = new JTextField();
				armorUpdateTextField.setColumns(10);
				updatePanel.add(armorUpdateTextField);
		...
		deletePanel.setLayout(new GridLayout(1,1));
				...
				lbldeleteWhere.setBounds(26, 50, 200, 50);
	...
	public void actionPerformed(ActionEvent e)
	{
		...
		if(e.getSource() == btnInsert)
		{
			...
			if(aridTextField.getText().equals("") || gidTextField.getText().equals("") ||
					protectionTextField.getText().equals("") || armorTextField.getText().equals("")||
		...
		else if(e.getSource() == btnUpdate)//event triggered when btnUpdate object is triggered
		{
			...
			if(aridUpdateTextField.getText().equals("") && gidUpdateTextField.getText().equals("") &&
					protectionUpdateTextField.getText().equals("") && armorUpdateTextField.getText().equals("")&&
			...
			else
			{
				...
				try {
					...
					String update = "UPDATE ARMOR SET "; 			
					if (!aridUpdateTextField.getText().equals(""))
					{
						String arid=aridUpdateTextField.getText();
						update+="ArID = "+ arid +" "; 
					...
					{
						update = "UPDATE ARMOR SET "; 
						String gid=gidUpdateTextField.getText();
						update+="GID = "+ gid +" "; 
						update+="WHERE "+ whereUpdateTextField.getText()+ ""; 
						stmt.executeUpdate(update);
					}
					if(!protectionUpdateTextField.getText().equals(""))
					{
						update = "UPDATE ARMOR SET "; 
						String protection=protectionUpdateTextField.getText();
						update+="Protection_Amount = "+ protection +" ";
						update+="WHERE "+ whereUpdateTextField.getText()+ "";
						stmt.executeUpdate(update);
					}
					if(!armorUpdateTextField.getText().equals(""))
					{
						update = "UPDATE ARMOR SET "; 
						String armor=armorUpdateTextField.getText(); 
						update+="Armor_Location = '"+ armor +"' ";
						update+="WHERE "+ whereUpdateTextField.getText()+ "";
						stmt.executeUpdate(update);
					}
		...
		else if(e.getSource() == btnDelete)//event triggered when btnDelete object is triggered
		{
			...
			else
			{
				try {
					Statement stmt = conn.createStatement(); 
					String delete = "DELETE FROM ARMOR ";
